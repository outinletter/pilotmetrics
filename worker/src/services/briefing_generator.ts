...
function generateHeuristicFactors(e: EventRow): string[] {
  ...
  // 2. Operational Hazards (Core Events)
  if (haystack.includes("UNSTABLE") || haystack.includes("STABILIZED")) factors.push("Unstable Approach Path");
  ...
  if (haystack.includes("EXCURSION") || haystack.includes("VEER")) factors.push("Runway Excursion Hazard");
  ...
  // 4. System/Technical
  if (haystack.includes("ENGINE FAILURE") || haystack.includes("ENGINE SHUTDOWN") || haystack.includes("POWER LOSS")) factors.push("Engine Power Loss");
  if (haystack.includes("FIRE") || haystack.includes("SMOKE") || haystack.includes("FUMES")) factors.push("Fire / Smoke Warning");
  if (haystack.includes("HYDRAULIC")) factors.push("Hydraulic System Degradation");
  if (haystack.includes("LANDING GEAR") || haystack.includes(" GEAR ")) factors.push("Landing Gear Issue");
  if (haystack.includes("MAINTENANCE") || haystack.includes("REPAIR")) factors.push("Maintenance-Linked Event");

  // 5. Human Factors
  if (haystack.includes("FATIGUE") || haystack.includes("TIRED")) factors.push("Crew Fatigue Factor");
  if (haystack.includes("COMMUNICATION") || haystack.includes("MISUNDERSTANDING")) factors.push("Communication Breakdown");

  return factors.length > 0 ? factors : ["[Pending Deep Analysis] Operational risk identified via route/phase match"];
}
...
function generateHeuristicLessons(e: EventRow): string[] {
  ...
  if (phase === "LANDING" || phase === "APPROACH") {
    lessons.push("Review stabilized approach criteria; go-around is mandatory if unstable below 1,000 ft AAL.");
    lessons.push("Enhance terrain awareness during approach; verify minimum sector altitudes (MSA).");
  } else if (phase === "TAKEOFF" || phase === "CLIMB") {
    lessons.push("Verify takeoff performance calculations; be prepared for rejected takeoff (RTO) before V1.");
  }

  if (haystack.includes("FATAL") || haystack.includes("DESTROYED")) {
    lessons.push("Maintain strictly disciplined crew coordination (CRM) in high-workload scenarios.");
  }

  if (haystack.includes("FIRE") || haystack.includes("SMOKE")) {
    lessons.push("Immediate execution of Memory Items for Fire/Smoke; land at nearest suitable airport without delay.");
  }
  ...
  return lessons.length > 0 ? lessons : ["Focus on standard operating procedures (SOP) and maintain situational awareness."];
}
