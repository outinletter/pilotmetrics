...
app.get("/api/briefing/:flightNumber", async c => {
  ...
  if (/^[A-Z]{3}$/.test(raw) || /^[A-Z]{4}$/.test(raw)) {
    ...
    const [weather, weatherMessages] = arrIcao
    ...
    const tags = [
      ...new Set([
        ...parseWeatherTags(weather.metar, weather.taf, arrIcao),
        ...fixedRisks,
      ]),
    ];
    ...
    const context: Record<string, unknown> = {
      ...
      arrival_weather_brief: arrivalWeatherBrief(
        weather.taf,
        weather.metar,
        null,
        0
    ...
    const hasNotam =
      arrIcao &&
      (c.env.NMS_CLIENT_ID || c.env.FAA_NOTAM_API_KEY);
  ...
  const [flight, flightMsg] = await getFlight(
    fn,
    c.env.AVIATIONSTACK_API_KEY,
    c.env.AIRPORTAL_SERVICE_KEY
  );

  const depIcao = iataToIcao(
    flight.departure_iata as string
  );

  const arrIcao = iataToIcao(
    flight.arrival_iata as string
  );

  const depIata =
    (flight.departure_iata as string) ?? "UNKNOWN";

  const arrIata =
    (flight.arrival_iata as string) ?? "UNKNOWN";
  ...
  const arrivalTime =
    (flight.estimated_arrival ??
      flight.scheduled_arrival) as string | null;

  const arrivalTaf = selectArrivalTafSegment(
    weather.taf,
    arrivalTime
  );
  ...
  const utcOffset = airportUtcOffset(
    arrIcao,
    arrivalDate
  );

  const nightArr = isNightArrival(
    arrivalTime,
    utcOffset
  );
  ...
  const metarTags = parseWeatherTags(
    weather.metar,
    "",
    arrIcao
  );
  ...
  const activeTags =
    arrivalTags.length > fixedRisks.length
      ? arrivalTags
      : tags;
  ...
  if (flightMsg) {
    ...
    context.flight_search_links = [
      {
        label: `${fn} flight status`,
        url: `https://www.google.com/search?q=${enc(
          `${fn} flight status`
        )}`,
      },
      {
        label: `${fn} ${context.route} today flight`,
        url: `https://www.google.com/search?q=${enc(
  ...
  // 브리핑에 포함된 이벤트들 중 분석이 안 된 것들 LLM 처리
  if (
    c.env.AI &&
    threats.length > 0
  ) {
